...
export async function up(db: Kysely<unknown>): Promise<void> {
  await sql`
    CREATE TABLE IF NOT EXISTS user_editor_preferences (
      ...
      last_opened_project_id uuid REFERENCES projects(id) ON DELETE SET NULL,
      ...
      CONSTRAINT user_editor_preferences_target_hierarchy_check CHECK (
        ...
        OR (last_opened_project_id IS NULL AND last_opened_diagram_id IS NULL)
      ),
      CONSTRAINT user_editor_preferences_diagram_requires_project_check CHECK (
        last_opened_diagram_id IS NULL OR last_opened_project_id IS NOT NULL
      )
    );

    CREATE INDEX IF NOT EXISTS user_editor_preferences_last_opened_project_id_idx
      ON user_editor_preferences(last_opened_project_id)
      WHERE last_opened_project_id IS NOT NULL;
...
export async function down(db: Kysely<unknown>): Promise<void> {
  await sql`
    ...
    DROP INDEX IF EXISTS user_editor_preferences_last_opened_project_id_idx;

...
export function Input({ className, onFocus, ...props }: InputProps) {
  ...
  return (
    <input
      className={cn(
        'flex h-[var(--tabliodb-control-md)] w-full rounded-[var(--tabliodb-radius-md)] border border-[rgb(var(--tabliodb-border-strong))] bg-white px-3 text-[13px] font-semibold leading-none text-[rgb(var(--tabliodb-ink))] outline-none transition placeholder:text-[rgb(var(--tabliodb-ink-subtle))] focus:border-[rgb(var(--tabliodb-primary))] focus:ring-[3px] focus:ring-[rgb(var(--tabliodb-focus-ring))] disabled:cursor-not-allowed disabled:opacity-50',
        className,
      )}
      onFocus={handleFocus}
      {...props}
